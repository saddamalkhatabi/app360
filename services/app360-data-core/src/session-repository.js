...
async function ensureExternalUser(db,identity){
  ...
  if(identity.account_external_id){
    ...
    if(!acct)await db('accounts').insert({id:accountId,type:identity.role==='trainer'?'institution':'family',name:identity.account_name||'حساب الريلز الآمن 360',status:'active',metadata_json:safeJson({external_account_id:identity.account_external_id,source:source})});
    await db('account_members').insert({account_id:accountId,user_id:userId,relationship:identity.role==='trainer'?'trainer':'learner',permissions_json:safeJson(['practice'])}).onConflict(['account_id','user_id']).merge();
  }
  user=await db('users').where({id:userId}).first();
...
async function importUsageBatch(db,userId,batch){
  ...
  await db.transaction(async function(trx){
    for(var i=0;i<entries.length;i++){
      var e=entries[i]||{};
      var appId=String(e.app_id||'');
      var day=String(e.occurred_day||now().slice(0,10)).slice(0,10);
      var seconds=positive(e.active_seconds,86400);
      var opens=positive(e.opens,1000);
      var receiptId=batchId+':'+appId+':'+day;
      if(!appId)continue;
      ...
      if(row){
        await trx('app_usage_daily').where({user_id:userId,app_id:appId,day:day}).update({active_seconds:(row.active_seconds||0)+seconds,opens:(row.opens||0)+opens,last_seen_at:now(),updated_at:now()});
      }else{
        await trx('app_usage_daily').insert({user_id:userId,app_id:appId,day:day,active_seconds:seconds,opens:opens,last_seen_at:now(),updated_at:now()});
      }
      saved++;
    }
  });
...
async function usageDashboard(db,userId){
  ...
  for(var i=0;i<rows.length;i++){
    rows[i].active_seconds=Number(rows[i].active_seconds)||0;
    rows[i].opens=Number(rows[i].opens)||0;
    total+=rows[i].active_seconds;
  }
  return{user_id:userId,total_active_seconds:total,apps:rows};
}

async function upsertUserContentBatch(db,userId,batch){
  batch=batch||{};
  ...
  await db.transaction(async function(trx){
    for(var i=0;i<items.length;i++){
      var x=items[i]||{};
      var appId=String(x.app_id||'');
      var clientKey=String(x.client_key||'');
      var type=String(x.content_type||'user_content');
      var title=String(x.title||'محتوى شخصي').slice(0,240);
      if(!appId||!clientKey)continue;
      var app=await trx('applications').where({id:appId}).first();
      if(!app)continue;
      var existing=await trx('content_items').where({owner_user_id:userId,app_id:appId,client_key:clientKey}).first();
      var content=safeJson(x.content||{});
      var id=existing?existing.id:internalId('uc',userId+'|'+appId+'|'+clientKey);
      var rev=existing?(existing.current_revision||1)+1:1;
      if(existing){
        await trx('content_items').where({id:id}).update({content_type:type,title:title,status:'draft',source_kind:'user-created',current_revision:rev,content_json:content,tags_json:safeJson(x.tags||[]),visibility:'private',origin_device_id:batch.device_id||null,updated_at:now()});
      }else{
        await trx('content_items').insert({id:id,app_id:appId,content_type:type,owner_user_id:userId,title:title,status:'draft',source_kind:'user-created',current_revision:1,content_json:content,tags_json:safeJson(x.tags||[]),client_key:clientKey,visibility:'private',moderation_status:'not_submitted',origin_device_id:batch.device_id||null});
      }
      await trx('content_revisions').insert({content_item_id:id,revision:rev,changed_by_user_id:userId,change_kind:existing?'user-sync-update':'user-sync-create',content_json:content,change_note:'private account sync'});
      saved++;
    }
  });
  return{saved:saved};
...
async function listUserContent(db,userId,appId){
  var q=db('content_items').where({owner_user_id:userId});
  if(appId)q=q.andWhere({app_id:appId});
  var rows=await q.orderBy('updated_at','desc').limit(200);
  return rows.map(function(r){
    var x={};
    for(var k in r)x[k]=r[k];
    try{x.content=JSON.parse(r.content_json||'{}')}catch(e){x.content={}}
    delete x.content_json;
    return x;
  });
}

module.exports={hashToken:hash,ensureExternalUser:ensureExternalUser,createIdentitySession:createIdentitySession,findIdentitySession:findIdentitySession,revokeIdentitySession:revokeIdentitySession,importUsageBatch:importUsageBatch,usageDashboard:usageDashboard,upsertUserContentBatch:upsertUserContentBatch,listUserContent:listUserContent};
