...
requiredApps.forEach(id=>{const a=byId[id];if(!a){fail('missing AI contract '+id);return}const index='apps/'+a.age_group+'/'+a.slug+'/index.html';if(!exists(index)){fail('AI app entry missing '+index);return}const html=read(index);let loader=html.indexOf('legacy-compat.js')>=0||html.indexOf('ai-content-studio.js')>=0;if(!loader&&(id==='a1-screen-move'||id==='a1-drawing-writing'))loader=sharedAudio.indexOf('ai-content-studio.js?v=1')>=0;if(!loader)fail('AI runtime loader missing from '+index);else ok('AI-enabled app '+id)});
