...
app.branding.theme_color = palette[age] || '#0f8f8a';
...
manifest.theme_color = palette[age] || '#0f8f8a';
...
html = html.replace(/<meta name="theme-color" content="[^"]+">/, `<meta name="theme-color" content="${palette[age] || '#0f8f8a'}">`);
html = html.replace(/\.head\{background:#0f8f8a/, `.head{background:${palette[age] || '#0f8f8a'}`);
...
icon = icon.replace('<title id="title">اسم التطبيق</title>', `<title id="title">${xmlTitle}</title>`)
  .replace('fill="#0f8f8a"','fill="'+(palette[age] || '#0f8f8a')+'"')
...
console.log('PWA ready: unique icon + manifest + offline shell + update helper.');
