var CACHE='app360-portal-v3';
var CORE=[
  './','./index.html','./manifest.webmanifest',
  './assets/css/lab360.css?v=1','./assets/css/lab360.css?v=2',
  './assets/js/lab360.js?v=1','./assets/js/lab360.js?v=2','./assets/js/pwa-install.js?v=1',
  './assets/brand/app360-lab-icon.svg','./assets/brand/app360-lab-icon-192.png','./assets/brand/app360-lab-icon-512.png','./assets/brand/app360-lab-logo.svg',
...
self.addEventListener('install',function(e){
  e.waitUntil(caches.open(CACHE).then(function(c){return Promise.all(CORE.map(function(u){return c.add(u).catch(function(){return null})}))}));
});
self.addEventListener('activate',function(e){
  e.waitUntil(caches.keys().then(function(keys){return Promise.all(keys.map(function(k){if(k.indexOf('app360-portal-')===0&&k!==CACHE)return caches.delete(k)}))}).then(function(){return self.clients&&self.clients.claim?self.clients.claim():null}));
});
...
self.addEventListener('fetch',function(e){
  ...
  var url=req.url||'',isData=url.indexOf('/data/catalog.json')>=0||url.indexOf('/data/goals.json')>=0;
