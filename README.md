...
المصدر المستخدم للشعار في البوابة هو `assets/brand/app360-lab-logo.webp`، مع أيقونات PNG الحالية. تبقى الهوية والأصول المعتمدة كما هي.
...
## خطط التطبيقات وشرح الأهداف

اضغط عدد الأهداف في أي بطاقة لعرض اسم كل هدف ودوره، والمساهمة العملية ودليل الملاحظة. زر «خطة التطبيق» يعرض الناتج ونطاق النسخة الأولى والقبول. يبحث الحقل أيضًا بعناوين الأهداف العربية.

توجد خطة لكل تطبيق في مساره الرسمي؛ المجلد المخطط لا يعرض رابط تشغيل. راجع [دليل بناء التطبيقات](docs/APP_BUILD_INDEX_AR.md)، ثم BUILD_SPEC.md وPROMPT_AR.md داخل المجلد. الرسم الحالي محفوظ دون تغيير تشغيله.

تُعدل الخطط والارتباطات في `data/catalog.json` ثم يشغل `pnpm blueprints`. لا تعدل الملفات المشتقة يدويًا. تدعم أداة `new:app` استكمال مجلد خطة موجود، مع حفظ هويته ووثائقه ومنع الكتابة فوق تطبيق بدأ تنفيذه.
