...
test('cards use storyboard first-frame thumbnails when available',()=>{assert.ok(js.includes('function cardVisual(x)'));assert.ok(js.includes('card-thumb-frame'));assert.equal(manifest.assets['001'],'storyboards/batch1/001.webp')});
test('index points to v16 runtime and style',()=>{assert.ok(html.includes('app-v16.js?v=4'));assert.ok(html.includes('styles-v16.css?v=4'))});
