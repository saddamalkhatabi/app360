...
if(index.indexOf('calm-with-me-v8-20260923')<0)fail('v8 marker missing');
...
if(index.indexOf('schoolView')<0||index.indexOf('مدرسة المشاعر')<0)fail('emotion school must remain inside calm app');
if(app.interoperability&&app.interoperability.embedded_planner!==false)fail('planner must not be embedded in calm app');
if(sw.indexOf('app360-app-calm-with-me-v8')<0||sw.indexOf('scenario-engine-v6')>=0||sw.indexOf('app-links-registry')>=0)fail('calm service worker must remain separated from cross-app planner assets');
console.log('calm-with-me v8 planner separation + emotion school validation OK');
