(function(w,d){
'use strict';
var down=false,moved=false,startX=0,startY=0,suppressUntil=0,coverFixTimer=null,coverFixRuns=0;
...
function fixSensoryCover(){var imgs=d.getElementsByTagName('img'),i,src;for(i=0;i<imgs.length;i++){src=imgs[i].getAttribute?String(imgs[i].getAttribute('src')||''):'';if(src.indexOf('assets/covers/a1-sensory.svg')>=0){imgs[i].src='assets/covers/a1-sensory.jpg?v=33';imgs[i].setAttribute('data-cover-fixed','sensory-jpg')}}coverFixRuns++;if(coverFixRuns>24&&coverFixTimer){clearInterval(coverFixTimer);coverFixTimer=null}}
function bootCoverFix(){fixSensoryCover();coverFixTimer=setInterval(fixSensoryCover,500);if(w.MutationObserver){try{var mo=new MutationObserver(function(){fixSensoryCover()});mo.observe(d.documentElement||d.body,{childList:true,subtree:true})}catch(e){}}}
if(d.addEventListener){d.addEventListener('touchstart',start,true);d.addEventListener('touchmove',move,true);d.addEventListener('touchend',end,true);d.addEventListener('touchcancel',end,true);d.addEventListener('mousedown',start,true);d.addEventListener('mousemove',move,true);d.addEventListener('mouseup',end,true);d.addEventListener('click',click,true);if(d.readyState==='loading')d.addEventListener('DOMContentLoaded',bootCoverFix,false);else bootCoverFix()}else{setTimeout(bootCoverFix,0)}
})(window,document);
