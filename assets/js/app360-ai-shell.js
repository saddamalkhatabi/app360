...
function stop(e){if(!e)return;if(e.preventDefault)e.preventDefault();if(e.stopImmediatePropagation)e.stopImmediatePropagation();else if(e.stopPropagation)e.stopPropagation();e.cancelBubble=true;return false}
function point(e){var t=e&&e.touches&&e.touches[0];if(!t)t=e&&e.changedTouches&&e.changedTouches[0];return t?{x:t.clientX||0,y:t.clientY||0}:null}
function installTouchGuard(){
 if(!legacy()||!d.addEventListener||w.APP360_TOUCH_GUARD)return;
 var st={active:false,moved:false,x:0,y:0,suppressUntil:0};
 function reset(){st.active=false;st.moved=false;st.x=0;st.y=0}
 d.addEventListener('touchstart',function(e){var p=point(e);if(!p){reset();return}st.active=true;st.moved=false;st.x=p.x;st.y=p.y},true);
 d.addEventListener('touchmove',function(e){var p;if(!st.active)return;p=point(e);if(!p)return;if(Math.abs(p.x-st.x)>=TOUCH_THRESHOLD||Math.abs(p.y-st.y)>=TOUCH_THRESHOLD)st.moved=true},true);
 d.addEventListener('touchend',function(e){if(st.active&&st.moved){st.suppressUntil=Date.now()+CLICK_BLOCK_MS;reset();stop(e);return}reset()},true);
 d.addEventListener('touchcancel',function(){reset()},true);
...
function loadEarlyChildSafety(){var p=(w.location&&w.location.pathname)||'',h,s;if(p.indexOf('/apps/1-4/calm-with-me/')<0||d.getElementById('app360EarlyChildSafety'))return;h=d.head||d.getElementsByTagName('head')[0]||d.body;if(!h)return;s=d.createElement('script');s.id='app360EarlyChildSafety';s.src='../../../assets/js/early-child-safety-v1.js?v=1';s.async=true;h.appendChild(s)}
