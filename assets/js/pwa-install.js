...
function promptInstall(){
  if(deferredPrompt){try{deferredPrompt.prompt();deferredPrompt.userChoice.then(function(){deferredPrompt=null;show(id('installPortalBtn'),false)})}catch(e){}}
  else if(standalone())note('التطبيق مثبت بالفعل على هذا الجهاز.','good');
  else note('من قائمة المتصفح اختر «تثبيت التطبيق» أو «إضافة إلى الشاشة الرئيسية» إذا لم يظهر زر التثبيت تلقائياً.','');
}
function activateWaiting(){try{if(reg&&reg.waiting)reg.waiting.postMessage({type:'SKIP_WAITING'})}catch(e){}}
function checkUpdate(manual){
  if(!reg){if(manual)note('خدمة التحديث غير متاحة في هذا المتصفح.','warn');return}
  if(manual)note('جارٍ التحقق من أحدث إصدار…','');
  try{reg.update().then(function(){if(reg.waiting)activateWaiting();else if(manual)note('أنت تستخدم أحدث إصدار متاح.','good')}).catch(function(){if(manual)note('تعذر التحقق الآن. سيعاد المحاولة تلقائياً عند توفر الاتصال.','warn')})}catch(e){if(manual)note('تعذر التحقق الآن.','warn')}
}
window.addEventListener('beforeinstallprompt',function(e){try{e.preventDefault();deferredPrompt=e;show(id('installPortalBtn'),!standalone())}catch(x){}});
window.addEventListener('appinstalled',function(){deferredPrompt=null;show(id('installPortalBtn'),false);note('تم تثبيت مختبر التطبيق 360 بنجاح.','good')});
function bind(){
  var b=id('installPortalBtn'),u=id('checkUpdateBtn');if(b)b.onclick=promptInstall;if(u)u.onclick=function(){checkUpdate(true)};
  ...
  var last=0;function periodic(){var now=+new Date();if(now-last<600000)return;last=now;checkUpdate(false)}
  window.addEventListener('focus',periodic,false);d.addEventListener('visibilitychange',function(){if(!d.hidden)periodic()},false);
}
